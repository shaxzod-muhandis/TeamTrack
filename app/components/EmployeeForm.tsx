...
export default function EmployeeForm({ defaultValues, onSubmit }: EmployeeFormProps) {
    const {
        ...
        setValue,
        formState: { errors },
    } = useForm<Omit<Employee, "id" | "created_at">>({
        defaultValues: {
            first_name: "",
            last_name: "",
            phone_primary: "",
            ...
            schedule: weekDays.reduce(
                (acc, day) => ({ ...acc, [day]: { from: "08:00", to: "18:00" } }),
                {}
            ),
            ...defaultValues,
        },
    ...
    useEffect(() => {
        ...
        else if (salaryType === "Soatlik") setValue("salary_type", "Oylik");
    }, [workType, salaryType, setValue]);
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <CreatableSelect
                            {...field}
                            options={branches}
                            onChange={(val) => field.onChange((val as Option | null)?.value ?? "")}
